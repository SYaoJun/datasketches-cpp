...
// catch2 testing framework provides such output, but this may be easier for
debugging
...
static hll_sketch build_sketch(uint8_t lg_k, target_hll_type hll_type,
                               hll_mode mode) {
  hll_sketch sk(lg_k, hll_type);
  int n = get_n(lg_k, mode);
  for (int i = 0; i < n; i++) {
    sk.update(static_cast<uint64_t>(i + v));
  }
  v += n;
  return sk;
}

// merges a sketch to an empty union and gets result of the same type, checks
// binary equivalence
static void union_one_update(bool compact) {
  for (uint8_t lg_k = 4; lg_k <= 21; lg_k++) { // all lg_k
    for (int mode = 0; mode <= 2; mode++) {    // List, Set, Hll
      if ((lg_k < 8) && (mode == 1)) {
        continue;
      } // lg_k < 8 list transitions directly to HLL
      for (int t = 0; t <= 2; t++) { // HLL_4, HLL_6, HLL_8
        target_hll_type hll_type = static_cast<target_hll_type>(t);
        hll_sketch sk1 =
            build_sketch(lg_k, hll_type, static_cast<hll_mode>(mode));
        hll_union u(lg_k);
        u.update(sk1);
        hll_sketch sk2 = u.get_result(hll_type);
        auto bytes1 =
            compact ? sk1.serialize_compact() : sk1.serialize_updatable();
        auto bytes2 =
            compact ? sk2.serialize_compact() : sk2.serialize_updatable();
...
TEST_CASE("hll isomorphic: union one update serialize updatable",
          "[hll_isomorphic]") {
  union_one_update(false);
}

TEST_CASE("hll isomorphic: union one update serialize compact",
          "[hll_isomorphic]") {
  union_one_update(true);
}

// converts a sketch to a different type and converts back to the original type
// to check binary equivalence
  ...
  for (uint8_t lg_k = 4; lg_k <= 21; lg_k++) { // all lg_k
    for (int mode = 0; mode <= 2; mode++) {    // List, Set, Hll
      ...
      for (int t1 = 0; t1 <= 2; t1++) { // HLL_4, HLL_6, HLL_8
        target_hll_type hll_type1 = static_cast<target_hll_type>(t1);
        hll_sketch sk1 =
            build_sketch(lg_k, hll_type1, static_cast<hll_mode>(mode));
        auto bytes1 =
            compact ? sk1.serialize_compact() : sk1.serialize_updatable();
        for (int t2 = 0; t2 <= 2; t2++) { // HLL_4, HLL_6, HLL_8
          if (t2 == t1)
            continue;
          target_hll_type hll_type2 = static_cast<target_hll_type>(t2);
...
TEST_CASE("hll isomorphic: convert back and forth serialize updatable",
          "[hll_isomorphic]") {
  convert_back_and_forth(false);
}

TEST_CASE("hll isomorphic: convert back and forth serialize compact",
          "[hll_isomorphic]") {
