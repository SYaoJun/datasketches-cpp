...
template <typename A>
HllArray<A>::HllArray(uint8_t lgConfigK, target_hll_type tgtHllType,
                      bool startFullSize, const A &allocator)
    : HllSketchImpl<A>(lgConfigK, tgtHllType, hll_mode::HLL, startFullSize),
      hipAccum_(0.0), kxq0_(1 << lgConfigK), kxq1_(0.0), hllByteArr_(allocator),
      curMin_(0), numAtCurMin_(1 << lgConfigK), oooFlag_(false),
...
HllArray<A>::HllArray(const HllArray &other, target_hll_type tgtHllType)
    : HllSketchImpl<A>(other.getLgConfigK(), tgtHllType, hll_mode::HLL,
                       other.isStartFullSize()),
      // remaining fields are initialized to empty sketch defaults
      // and left to subclass constructor to populate
      hipAccum_(0.0), kxq0_(1 << other.getLgConfigK()), kxq1_(0.0),
      hllByteArr_(other.getAllocator()), curMin_(0),
      numAtCurMin_(1 << other.getLgConfigK()), oooFlag_(false),
      rebuild_kxq_curmin_(false) {}
...
HllArray<A> *HllArray<A>::newHll(const void *bytes, size_t len,
  ...
  const hll_mode mode =
      HllSketchImpl<A>::extractCurMode(data[hll_constants::MODE_BYTE]);
  if (mode != HLL) {
  ...
  const target_hll_type tgtHllType =
      HllSketchImpl<A>::extractTgtHllType(data[hll_constants::MODE_BYTE]);
  const bool oooFlag =
      ((data[hll_constants::FLAGS_BYTE] & hll_constants::OUT_OF_ORDER_FLAG_MASK)
           ? true
           : false);
  const bool comapctFlag =
      ((data[hll_constants::FLAGS_BYTE] & hll_constants::COMPACT_FLAG_MASK)
           ? true
           : false);
  const bool startFullSizeFlag =
      ((data[hll_constants::FLAGS_BYTE] & hll_constants::FULL_SIZE_FLAG_MASK)
           ? true
           : false);
  ...
  std::memcpy(&numAtCurMin, data + hll_constants::CUR_MIN_COUNT_INT,
              sizeof(int));
  std::memcpy(&auxCount, data + hll_constants::AUX_COUNT_INT, sizeof(int));

  AuxHashMap<A> *auxHashMap = nullptr;
  typedef std::unique_ptr<AuxHashMap<A>, std::function<void(AuxHashMap<A> *)>>
      aux_hash_map_ptr;
  ...
  if (auxCount > 0) { // necessarily TgtHllType == HLL_4
    ...
    auxHashMap =
        AuxHashMap<A>::deserialize(auxDataStart, len - offset, lgK, auxCount,
                                   auxLgIntArrSize, comapctFlag, allocator);
    aux_ptr = aux_hash_map_ptr(auxHashMap, auxHashMap->make_deleter());
  }

  HllArray<A> *sketch = HllSketchImplFactory<A>::newHll(
      lgK, tgtHllType, startFullSizeFlag, allocator);
  ...
  std::memcpy(sketch->hllByteArr_.data(),
              data + hll_constants::HLL_BYTE_ARR_START, arrayBytes);

  if (auxHashMap != nullptr) {
    static_cast<Hll4Array<A> *>(sketch)->putAuxHashMap(auxHashMap);
  }
...
HllArray<A> *HllArray<A>::newHll(std::istream &is, const A &allocator) {
  ...
  if (listHeader[hll_constants::PREAMBLE_INTS_BYTE] !=
      hll_constants::HLL_PREINTS) {
    throw std::invalid_argument("Incorrect number of preInts in input stream");
  }
  ...
  hll_mode mode =
      HllSketchImpl<A>::extractCurMode(listHeader[hll_constants::MODE_BYTE]);
  if (mode != HLL) {
    throw std::invalid_argument(
        "Calling HLL constructor with non-HLL mode data");
  }

  const target_hll_type tgtHllType =
      HllSketchImpl<A>::extractTgtHllType(listHeader[hll_constants::MODE_BYTE]);
  const bool oooFlag = ((listHeader[hll_constants::FLAGS_BYTE] &
                         hll_constants::OUT_OF_ORDER_FLAG_MASK)
                            ? true
                            : false);
  const bool comapctFlag = ((listHeader[hll_constants::FLAGS_BYTE] &
                             hll_constants::COMPACT_FLAG_MASK)
                                ? true
                                : false);
  const bool startFullSizeFlag = ((listHeader[hll_constants::FLAGS_BYTE] &
                                   hll_constants::FULL_SIZE_FLAG_MASK)
                                      ? true
                                      : false);
  ...
  HllArray *sketch = HllSketchImplFactory<A>::newHll(
      lgK, tgtHllType, startFullSizeFlag, allocator);
  typedef std::unique_ptr<HllArray<A>, std::function<void(HllSketchImpl<A> *)>>
      hll_array_ptr;
  ...
  if (auxCount > 0) { // necessarily TgtHllType == HLL_4
    ...
    AuxHashMap<A> *auxHashMap = AuxHashMap<A>::deserialize(
        is, lgK, auxCount, auxLgIntArrSize, comapctFlag, allocator);
    static_cast<Hll4Array<A> *>(sketch)->putAuxHashMap(auxHashMap);
  }
...
template <typename A>
auto HllArray<A>::serialize(bool compact,
                            unsigned header_size_bytes) const -> vector_bytes {
  const size_t sketchSizeBytes = (compact ? getCompactSerializationBytes()
  ...
  bytes[hll_constants::SER_VER_BYTE] =
      static_cast<uint8_t>(hll_constants::SER_VER);
  bytes[hll_constants::FAMILY_BYTE] =
      static_cast<uint8_t>(hll_constants::FAMILY_ID);
  bytes[hll_constants::LG_K_BYTE] = static_cast<uint8_t>(this->lgConfigK_);
  bytes[hll_constants::LG_ARR_BYTE] = static_cast<uint8_t>(
      auxHashMap == nullptr ? 0 : auxHashMap->getLgAuxArrInts());
  ...
  std::memcpy(bytes + hll_constants::HIP_ACCUM_DOUBLE, &hipAccum_,
              sizeof(double));
  ...
  std::memcpy(bytes + hll_constants::CUR_MIN_COUNT_INT, &numAtCurMin_,
              sizeof(uint32_t));
  const uint32_t auxCount =
      (auxHashMap == nullptr ? 0 : auxHashMap->getAuxCount());
  std::memcpy(bytes + hll_constants::AUX_COUNT_INT, &auxCount,
              sizeof(uint32_t));
  ...
  if (this->tgtHllType_ == HLL_4) {
    ...
    if (auxHashMap != nullptr) {
      if (compact) {
        ...
        std::memcpy(bytes, auxHashMap->getAuxIntArr(),
                    auxHashMap->getUpdatableSizeBytes());
      }
    } else if (!compact) {
      // if updatable, we write even if currently unused so the binary can be
      // wrapped
  ...
  if (this->tgtHllType_ == HLL_4) {
    if (auxHashMap != nullptr) {
      if (compact) {
        ...
        write(os, auxHashMap->getAuxIntArr(),
              auxHashMap->getUpdatableSizeBytes());
      }
    } else if (!compact) {
      // if updatable, we write even if currently unused so the binary can be
      // wrapped
      uint32_t auxBytes = 4 << hll_constants::LG_AUX_ARR_INTS[this->lgConfigK_];
      std::fill_n(std::ostreambuf_iterator<char>(os), auxBytes,
                  static_cast<char>(0));
    }
  }
}

template <typename A> double HllArray<A>::getEstimate() const {
  if (oooFlag_) {
...
/*
 * The upper and lower bounds are not symmetric and thus are treated slightly
 * differently. For the lower bound, when the unique count is <= k, LB >=
 * numNonZeros, where numNonZeros = k - numAtCurMin AND curMin == 0.
 *
 * For HLL6 and HLL8, curMin is always 0 and numAtCurMin is initialized to k and
 * is decremented down for each valid update until it reaches 0, where it stays.
 * Thus, for these two isomorphs, when numAtCurMin = 0, means the true curMin is
 * > 0 and the unique count must be greater than k.
 *
 * HLL4 always maintains both curMin and numAtCurMin dynamically. Nonetheless,
 * the rules for the very small values <= k where curMin = 0 still apply.
 */
template <typename A>
double HllArray<A>::getLowerBound(uint8_t numStdDev) const {
  ...
  const double numNonZeros =
      ((curMin_ == 0) ? (configK - numAtCurMin_) : configK);
  const double relErr =
      HllUtil<A>::getRelErr(false, this->oooFlag_, this->lgConfigK_, numStdDev);
...
double HllArray<A>::getUpperBound(uint8_t numStdDev) const {
  HllUtil<A>::checkNumStdDev(numStdDev);
  const double relErr =
      HllUtil<A>::getRelErr(true, this->oooFlag_, this->lgConfigK_, numStdDev);
...
template <typename A> double HllArray<A>::getCompositeEstimate() const {
  const double rawEst = getHllRawEstimate();

  const double *xArr =
      CompositeInterpolationXTable<A>::get_x_arr(this->lgConfigK_);
  const uint32_t xArrLen = CompositeInterpolationXTable<A>::get_x_arr_length();
  const double yStride =
      CompositeInterpolationXTable<A>::get_y_stride(this->lgConfigK_);
  ...
  double adjEst = CubicInterpolation<A>::usingXArrAndYStride(xArr, xArrLen,
                                                             yStride, rawEst);

  // We need to completely avoid the linear_counting estimator if it might have
  // a crazy value. Empirical evidence suggests that the threshold 3*k will keep
  // us safe if 2^4 <= k <= 2^21.

  if (adjEst > (3 << this->lgConfigK_)) {
    return adjEst;
  }

  const double linEst = getHllBitMapEstimate();

  // Bias is created when the value of an estimator is compared with a threshold
  // to decide whether to use that estimator or a different one. We conjecture
  // that less bias is created when the average of the two estimators is
  // compared with the threshold. Empirical measurements support this
  // conjecture.

  const double avgEst = (adjEst + linEst) / 2.0;

  // The following constants comes from empirical measurements of the crossover
  // point between the average error of the linear estimator and the adjusted
  // hll estimator
  double crossOver = 0.64;
  if (this->lgConfigK_ == 4) {
    crossOver = 0.718;
  } else if (this->lgConfigK_ == 5) {
    crossOver = 0.672;
  }
...
template <typename A> double HllArray<A>::getKxQ0() const { return kxq0_; }

template <typename A> double HllArray<A>::getKxQ1() const { return kxq1_; }

template <typename A> double HllArray<A>::getHipAccum() const {
  return hipAccum_;
}

template <typename A> uint8_t HllArray<A>::getCurMin() const { return curMin_; }

template <typename A> uint32_t HllArray<A>::getNumAtCurMin() const {
  return numAtCurMin_;
}

template <typename A> void HllArray<A>::putKxQ0(double kxq0) { kxq0_ = kxq0; }

template <typename A> void HllArray<A>::putKxQ1(double kxq1) { kxq1_ = kxq1; }

template <typename A> void HllArray<A>::putHipAccum(double hipAccum) {
  hipAccum_ = hipAccum;
}

template <typename A> void HllArray<A>::putCurMin(uint8_t curMin) {
  curMin_ = curMin;
}

template <typename A> void HllArray<A>::putNumAtCurMin(uint32_t numAtCurMin) {
  numAtCurMin_ = numAtCurMin;
}

template <typename A> bool HllArray<A>::isCompact() const { return false; }

template <typename A> bool HllArray<A>::isEmpty() const {
  const uint32_t configK = 1 << this->lgConfigK_;
  return (curMin_ == 0) && (numAtCurMin_ == configK);
}

template <typename A> void HllArray<A>::putOutOfOrderFlag(bool flag) {
  oooFlag_ = flag;
}

template <typename A> bool HllArray<A>::isOutOfOrderFlag() const {
  return oooFlag_;
}

template <typename A>
uint32_t HllArray<A>::hllArrBytes(target_hll_type tgtHllType,
                                  uint8_t lgConfigK) {
  switch (tgtHllType) {
  case HLL_4:
...
template <typename A> uint32_t HllArray<A>::hll4ArrBytes(uint8_t lgConfigK) {
  return 1 << (lgConfigK - 1);
}

template <typename A> uint32_t HllArray<A>::hll6ArrBytes(uint8_t lgConfigK) {
  const uint32_t numSlots = 1 << lgConfigK;
  return ((numSlots * 3) >> 2) + 1;
}

template <typename A> uint32_t HllArray<A>::hll8ArrBytes(uint8_t lgConfigK) {
  return 1 << lgConfigK;
}

template <typename A> uint32_t HllArray<A>::getMemDataStart() const {
...
uint32_t HllArray<A>::getCompactSerializationBytes() const {
  AuxHashMap<A> *auxHashMap = getAuxHashMap();
  const uint32_t auxCountBytes =
      ((auxHashMap == nullptr) ? 0 : auxHashMap->getCompactSizeBytes());
  return hll_constants::HLL_BYTE_ARR_START + getHllByteArrBytes() +
         auxCountBytes;
}

template <typename A> uint8_t HllArray<A>::getPreInts() const {
  return hll_constants::HLL_PREINTS;
}

template <typename A> AuxHashMap<A> *HllArray<A>::getAuxHashMap() const {
  return nullptr;
}
...
template <typename A>
void HllArray<A>::hipAndKxQIncrementalUpdate(uint8_t oldValue,
                                             uint8_t newValue) {
  const uint32_t configK = 1 << this->getLgConfigK();
  // update hip BEFORE updating kxq
  if (!oooFlag_)
    hipAccum_ += configK / (kxq0_ + kxq1_);
  // update kxq0 and kxq1; subtract first, then add
  if (oldValue < 32) {
    kxq0_ -= INVERSE_POWERS_OF_2[oldValue];
  } else {
    kxq1_ -= INVERSE_POWERS_OF_2[oldValue];
  }
  if (newValue < 32) {
    kxq0_ += INVERSE_POWERS_OF_2[newValue];
  } else {
    kxq1_ += INVERSE_POWERS_OF_2[newValue];
  }
}
...
template <typename A> double HllArray<A>::getHllBitMapEstimate() const {
  const uint32_t configK = 1 << this->lgConfigK_;
...
template <typename A> double HllArray<A>::getHllRawEstimate() const {
  const uint32_t configK = 1 << this->lgConfigK_;
  ...
  } else if (this->lgConfigK_ == 6) {
    correctionFactor = 0.709;
  } else {
    correctionFactor = 0.7213 / (1.0 + (1.079 / configK));
  }
  const double hyperEst =
      (correctionFactor * configK * configK) / (kxq0_ + kxq1_);
  return hyperEst;
}

template <typename A> void HllArray<A>::setRebuildKxqCurminFlag(bool rebuild) {
  rebuild_kxq_curmin_ = rebuild;
}

template <typename A> bool HllArray<A>::isRebuildKxqCurminFlag() const {
  return rebuild_kxq_curmin_;
}

template <typename A> void HllArray<A>::check_rebuild_kxq_cur_min() {
  if (!rebuild_kxq_curmin_) {
    return;
  }
  ...
  while (it != end) {
    ...
    if (v > 0) {
      if (v < 32) {
        kxq0 += INVERSE_POWERS_OF_2[v] - 1.0;
      } else {
        kxq1 += INVERSE_POWERS_OF_2[v] - 1.0;
      }
    }
    if (v > cur_min) {
      ++it;
...
typename HllArray<A>::const_iterator HllArray<A>::begin(bool all) const {
  return const_iterator(hllByteArr_.data(), 1 << this->lgConfigK_, 0,
                        this->tgtHllType_, nullptr, 0, all);
}

template <typename A>
typename HllArray<A>::const_iterator HllArray<A>::end() const {
  return const_iterator(hllByteArr_.data(), 1 << this->lgConfigK_,
                        1 << this->lgConfigK_, this->tgtHllType_, nullptr, 0,
                        false);
}
...
HllArray<A>::const_iterator::const_iterator(const uint8_t *array,
                                            ...
                                            uint8_t offset, bool all)
    : array_(array), array_size_(array_size), index_(index),
      hll_type_(hll_type), exceptions_(exceptions), offset_(offset), all_(all) {
  while (index_ < array_size_) {
    value_ = get_value(array_, index_, hll_type_, exceptions_, offset_);
    if (all_ || value_ != hll_constants::EMPTY)
      break;
    ++index_;
  }
}
...
template <typename A>
typename HllArray<A>::const_iterator &
HllArray<A>::const_iterator::operator++() {
  while (++index_ < array_size_) {
    value_ = get_value(array_, index_, hll_type_, exceptions_, offset_);
    if (all_ || value_ != hll_constants::EMPTY)
      break;
...
template <typename A>
bool HllArray<A>::const_iterator::operator!=(
...
uint8_t HllArray<A>::const_iterator::get_value(const uint8_t *array,
                                               ...
                                               const AuxHashMap<A> *exceptions,
                                               uint8_t offset) {
  // TODO: we should be able to improve efficiency here by reading multiple
  // bytes at a time
  //       for HLL4 and HLL6
...
template <typename A> A HllArray<A>::getAllocator() const {
